...
{	
	WebDriverWait wait => new WebDriverWait(App, TimeSpan.FromSeconds(8));
    ...
    {
		if(!AppiumServerHelper.IsWindowsValid() && !OperatingSystem.IsMacOS() || App is null)
		{
			return;
		}
		...
		// Click on Views
		var views = App.FindElement(ByAndroidUIAutomator.AndroidUIAutomator("new UiSelector().text(\"Views\")"));
		views.Click();

		// Scroll to MediaElement and click on it
		var mediaElement = App.FindElement(ByAndroidUIAutomator.AndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true).instance(0)).scrollIntoView(new UiSelector().text(\"MediaElement\"))"));
		mediaElement.Click();
		...
		var pauseBtn = App.FindElement(ByAndroidUIAutomator.AndroidUIAutomator("new UiSelector().resourceId(\"com.microsoft.CommunityToolkit.Maui.Sample:id/PauseBtn\")"));
		Assert.That(pauseBtn.Displayed, Is.True, "Pause button should be visible after playing");
		pauseBtn.Click();

		// Click on Play button
		var playBtn = App.FindElement(ByAndroidUIAutomator.AndroidUIAutomator("new UiSelector().resourceId(\"com.microsoft.CommunityToolkit.Maui.Sample:id/PlayBtn\")"));
		Assert.That(playBtn.Displayed, Is.True, "Play button should be visible");
		playBtn.Click();

		// Click on Stop button
		var stopBtn = App.FindElement(ByAndroidUIAutomator.AndroidUIAutomator("new UiSelector().resourceId(\"com.microsoft.CommunityToolkit.Maui.Sample:id/StopBtn\")"));
